...
/**
 * The registration of an AI bot player. The `player_id` matches the regular player entry in [`PropagatedState::players`]; this registry is what marks that seat as bot-controlled.
 */
...
export interface DrawPhase {
  ...
  decks?: Deck[];
  /**
   * The seats that have explicitly marked themselves "done bidding" (clicked the temporary "Done bidding" button). The bidding window stays open until EVERY human (non-bot) seat is in this set; a NEW bid clears it so everyone re-confirms in response. Bots are never added here — they count as implicitly done (see [`DrawPhase::all_humans_done_bidding`]). Defaulted so older serialized states (without the field) deserialize cleanly.
   */
  done_bidding?: number[];
  game_mode: GameMode;
  hands: Hands;
