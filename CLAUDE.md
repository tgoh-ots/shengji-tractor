...
- **frontend/static/rules.html** + **strategy.html**: the two standalone CSP-safe guide pages (copied to `dist/` by webpack; NOT compiled from `src/`; also reachable via the `/rules` and `/strategy` redirects). Both are language-aware (no Chinese in English mode) and render real SVG card examples. `rules.html` holds the rules + a "Finding Friends only" callout; `strategy.html` is the **strategy guide** — a CSS-only Basics/Advanced tab switch (`?tab=advanced` deep-links it) over the strategy the Enoch/Grandmaster tiers actually play, with worked example hands. **If you change the bots' strategy in a user-visible way, update `strategy.html` too.**
- **frontend/static/guide.css** + **guide-common.js**: shared by BOTH guide pages — the dark reading-column chrome, the nav/tab components, and the lang / top-nav / static-card-SVG helpers (`window.ShengjiGuide`). Per-page scripts are **rules.js** and **strategy.js**; card art comes from **rules-cards.json**. All page JS is external and same-origin (no inline `<script>`, no event-handler attributes) so the pages work under the production CSP, and both tab switches are CSS-only so they survive JS being disabled. NOTE: the language gate in `guide.css` uses `!important` deliberately — a component rule that sets `display` through a deeper selector would otherwise out-specify it and leak the hidden language.
